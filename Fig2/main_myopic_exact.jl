...
sim = NestedFilterSimulation(
        ...
        LinRange(0.05,1,30),
        1024, 256, 12,
        timestep = Myopic(candidates,0.0)
        )
...
function f2(data)
        save(string("Myopic_exact_",Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]),".jld"), "data", data)
end

rec = Recording(f1, f2, sim)

sim.hstate.n .= N
times, epsps = run_exact!(sim, T = 400, recording = Recording(f1, f2, sim))
