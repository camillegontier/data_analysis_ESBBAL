...
ISI_values_myopic = []
for j in 1:400
    obj = load(string("Myopic_",j,".jld"))
    data = obj["data"]
    ISI = diff(data[1][6])
    for i in 1:200
        append!(ISI_values_myopic,ISI[i])
    end
end
################################################

candidates = ISI_values_myopic
...
function f2(data)
        save(string("optimal_fixed_",Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]),".jld"), "data", data)
end
